    ...
    <div>
      ...
      <div className="detail">
        <img
          ...
          src={EventMusic.picture}
          alt={EventMusic.name}
        />
        <h1 className="eventName">Etkinlik Adı: {EventMusic.name}</h1>
        <h5>Şehir: {EventMusic.place.city}</h5>
        <h5>Yer: {EventMusic.place.location}</h5>
        <h5>Gün:{formatDateString(EventMusic.date)}</h5>
        <h5>Fiyat: {EventMusic.price} TL</h5>
        <span >
          <p className="mt-4"> 
        ...
        <button
          type="button"
          onClick={() => navigate(-1)}
          class="btn backgroundColor3 m-3"
        >
          Geri
        </button>
      </div>

...
const SiteDance = () => {
  ...
  const uniqueCities = Array.from(
    new Set(danceData.map((item) => item.place.city))
  );
  ...
  const filteredDanceData = danceData.filter((item) => {
    ...
    const startDateFilter =
      !startDate || new Date(item.date) >= new Date(startDate + "T00:00:00Z");
    const endDateFilter =
      !endDate || new Date(item.date) <= new Date(endDate + "T23:59:59Z");
  ...
  return (
    <div>
      ...
      <div className="d-flex flex-wrap justify-content-evenly ">
        {filteredDanceData.map((item) => (
          <div className="card" style={{ width: "18rem" }}>
              <Link to={`/dance/${item.id}`} >
                <img className='picture' src={item.picture} alt={item.name} />
                </Link>
            <div className="card-body">
              <h5 className="card-title">
                <Link to={`/dance/${item.id}`} className="text color2" >{item.name}</Link>
              </h5>
              <p className="card-text">
                <Link to={`/dance/${item.id}`} className="text color2">{item.place.city}</Link>
              </p>
              <p className="card-text">
                <Link to={`/dance/${item.id}`} className="text color2">{item.place.location}</Link>
              </p>
              <p className="card-text">
                <Link to={`/dance/${item.id}`} className="text color2">
                  {formatDateString(item.date)}
                </Link>
              </p>
              <p className="card-text">
                <Link to={`/dance/${item.id}`} className="text color2">{item.price} TL</Link>
              </p>
            </div>
          </div>
        ))}
